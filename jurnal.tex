...
%\title{Study of Bittorrent Swarms Topologies Properties with Peer Exchange Protocol}
%\title{Study of Bittorrent Swarms Topologies Properties}
%\title{Infering Bittorrent Swarms Topologies Properties using Peer Exchange Protocol}
\title{Infering Bittorrent Swarms Topologies Using Peer Exchange Protocol}
%
...
Every $30$ seconds the Bittorrent peer utilizes optimistic algorithm. 
...
Generaly, choking-unchoking algorithm also called as \textit{tit-for-tat}. 
...
The common method is to observed the system for long time and sample the peers randomly. 
This method will incorrectly conclude that most of the peers in the system have very few files.
The problem of this method is that it focuses on sampling number of peers instead of peer properties.
...
With that method, the sampling will not biased because we track same peer's property in different of time. 
...
Ho\ss{}feld \textit{et al}. \cite{TR464} discuss the AS (Autonomous System) level topology of Bittorrent swarm for optimizing overlay traffic across ASes, while our study focus on microspic dynamic aspect which is Bittorrent swarms topology itself (peer level or IP address level). 
The closest work to ours is Kryczka \textit{et al}. \cite{Kryczka2011}.
While our method is slightly same with theirs, they are focus on clustering and locality while our focus are on node degree and clustering.
...
Because of different PEX implementation between Bittorrent clients, we need to be carefull with it in data processing.  
In our work, we describe PEX behavior and its limitation on popular two Bittorrent clients on the Internet : Vuze and uTorrent, and we also explain how to treat with PEX data from different Bittorrent clients. 
We also provide simulation result to confirm that our methods to infer peer relationship with PEX is valid.  
They observed that Bittorrent swarms have slightly higher clustering coefficient compare to random graphs of the same size and they observe neither of Bittorrent swarms fulfill the properties of small world.
The slightly difference in clustering aspect come from the difference of PEX data processing. 
They assume that PEX is same and complete for all Bittorrent clients therefore they get slightly different result. 
...
From Bittorrent protocol point of view, the reason that a Bittorrent swarm can be described by a power-law with exponential cut-off is: leechers in a Bittorrent swarm prefer a few good seeders or neighbours that can give high data rate to exchange the data and seeders have rich connections to leechers as seeders have complete chunks or pieces. 
%we have been explained it in Sect.\ref{sec:background}. 
That behavior explains seeders have rich connections while leechers only have a few neighbours. 
We argue that there are two reasons for cut-off phonemenon. 
First, most Bittorrent clients configure the maximum global connection between $200-300$ however the maximum connection per torrent (swarm) is set between $50 - 90$ by default \cite{clientv}\cite{clientu}.
Even some Bitorrent forums suggest to decrease the maximum connection for torrent (swarm) to between $30-40$ \cite{clientf}. 
Second, most of the Bittorrent users are home user where their home gateway device can not give high concurrent connections and Bitorrent is not main online activity. 
We argue that the Bittorrent swarm closes to random that we infer from clustering coeficient is caused by Bittorrent mechanism itself that always choose random peers from its neighbours in choking-unchoking algorithm, optimistic choking algorithm, and optimistic connect algorithm as we explain on Sect.\ref{sec:background}.
