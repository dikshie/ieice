...
For illustrative: suppose we want to measure number of files shared by peers in Bittorrent swarm.
...
Thus, we must distinguish between occurrences of the same peer at different times: samples $v_{i,t}$ and $v_{i,t'}$ gathered at distinct times $t t'$ are viewed as distinct, even when they come from the sampe peer.
The key in this technique is that it must be possible to sample from the same peer more than once at different points in time.
Thus we can formalize this into $v_{i,t} \in V_t  , t \in [t_0, t_0 + \Delta]$.
With that technique, the sampling will not biased by the dynamics of peer behavior because we track same peer's property in different of time. 
