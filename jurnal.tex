...
%\title{Infering Bittorrent Swarms Topologies Properties with Peer Exchange Protocol}
\title{Infering Bittorrent Swarms Topologies with Peer Exchange Protocol}
%
...
In this Bittorrent swarm, half of the peers are up all the time and have many files, while the other peers remain around for one minute and are immediately replaced by new short lived peers who have few files.
The common technique is to observed the system for long time and sample the peers randomly. 
This technique will incorrectly conclude that most of the peers in the system have very few files.
The problem of this technique is that it focuses on sampling number of peers instead of peer properties.
%It selects each sampled nodes at most once.
%However, the property at the nodes may change with time.  
The goal should not be to select a vertex $v_i \in \bigcup^{t_0+\Delta}_{t=t_0} V_t$ but to sample the property of a vertex at $v_i$ at a particular instant time $t$.
...
From Bitorrent point of view, the reason that a Bittorrent swarm can be described by a power-law with exponential cut-off is: the leecher in Bittorrent prefer a few good seeders or neighbours that give high data rate to exchange the data that we have been explained in Sect.\ref{sec:background}. 
That explains few seeders or neighbours have rich connections to the peers while other peers only have a few neighbours. 
