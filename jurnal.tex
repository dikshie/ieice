...
From Bitorrent point of view, the reason that a Bittorrent swarm can be described by a power-law with exponential cut-off is: the leecher in Bittorrent prefer a few good seeders or neighbours that give high data rate to exchange the data that we have been explained in sec.\ref{sec:background}. 
...
First, while most Bittorrent clients configure the maximum global connection between $200-300$, the maximum connection per torrent (swarm) is set between $50 - 80$ by default. 
Even some Bitorrent forums suggest to decrease the maximum connection for torrent (swarm) to between $30-40$. 
...
We argur that the Bittorrent swarm closes to random that we infer from clustering coeficient is caused by Bittorrent mechanism itself that always choose random peers from its neighbours in choking-unchoking algorithm, optimistic choking algorithm, and optimistic connect algorithm as we explain on sec.\ref{sec:background}.
