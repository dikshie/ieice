...
In the nested case, we compare power-law model as a bigger family with power-law with exponential cut-off as smaller family, while in non-nested case, we campore power-law model with different distributions such as exponential or log-normal.
...
\subfloat[Scatter plot of $p$ value vs $\rho$ value. 
        Points in area 1 ($\rho$ value $<0.1$ and $p$ value $>0$) are the samples where a power-law with exponential cut-off is a more plausible model than a power-law.
...
\subfloat[Scatter plot of $p$ value vs log-likelihood ratio (LR) for $\rho < 0.1$. In this figure we define area 1: $LR>0$ and $p$ value $<0.1$, area 2: $LR>0$ and $p$ value $>0.1$, area 3: $LR<0$ and $p$ value $<0.1$, area 4: $LR<0$ and $p$ value $>0.1$. There are no points in area 1 and area 2, meaning that the power-law model is not a better model for all data; instead $42\%$ of the points lie in area 3 and $58\%$ of the points lie in area 4.]{
...
\begin{figure*}[!t]
\begin{center}
\subfloat[CDF plot of $\rho$ value of log-likehood ratio test for power-law vs log-normal. Only $13\%$ $\rho$ values are less than $0.1$ therefore log-likelihood ratio sign is reliable only for $13\%$.]{
...
For the other 87%, this test indicates that we cannot use the sign of the log-likelihood ratio to draw firm conclusions.
Figure \ref{fig:scatter-lognorm} shows the log-likehood ratio for the points where $\rho < 0.1$.  For these points, 
all log-likehood ratio values have negative signi, therefore log-normal is not a better distribution than power-law.

\begin{figure*}[!t]
\begin{center}
\subfloat[CDF plot of $\rho$ value of log-likehood ratio test for power-law vs exponential. Only $5.5\%$ $\rho$ values are less than $0.1$ therefore log-likelihood ratio sign is reliable only for $5.5\%$.]{
...
\begin{figure}[!tb]
\begin{center}
\includegraphics[scale=0.65]{graphs/new/cdf-clustering.eps}
\end{center}
...
\begin{figure}[!tb]
\begin{center}
\includegraphics[scale=0.65]{netscicom-graphs/simula.eps}
\end{center}
