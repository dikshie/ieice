...
P2P traffic dominated the Internet in 2008 is still growing even though recent studies suggest that its growth is slower than that, and its proportion to the Internet traffic is declining \cite{labovitz2010internet} \cite{index2010forecast}.
...
We found in the literature contradictory results of \textit{clustering},
Furthermore, we also study the clustering in Bittorrent swarms.
...
Both alternative distributions only show very small number of data points that have $\rho < 0.1$, each at $13\%$ and $5.5\%$.
For these data points the log-likelihood ratio have negative signs as shown in Fig. \ref{fig:scatter-lognorm} and Fig. \ref{fig:scatter-exp}, therefore the alternative distributions are not better than power-law.
