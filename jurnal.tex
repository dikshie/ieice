...
\captionsetup[subfigure]{labelformat=parens, justification=raggedright, format=hang, margin=3pt, parskip=0pt,
		hangindent=0pt, indention=0pt, singlelinecheck=false}
...
\affiliate[labelA]{The author is with Graduate School of Media and Governance Keio University, Fujisawa-shi, 252-8520 Japan }
\affiliate[labelB]{The author is with Faculty of Environment and Information Studies Keio University, Fujisawa-shi, 252-8520 Japan }
...
However, we still have little knowledge about the topology of real Bittorrent swarms and how the topology tell us about peers dynamic in the Bittorrent swarm.
...
\begin{figure}[tb]
\begin{center}
\includegraphics[scale=0.6]{graphs/new/cdf-num.eps}
\end{center}
...
\begin{figure}[tb]
\begin{center}
\includegraphics[scale=0.6]{graphs/output_2.eps}
\end{center}
...
\includegraphics[scale=0.6]{graphs/new/aggregate-p-cdf.eps}
\end{center}
...
%\begin{figure}[tb]
%\begin{center}
%\includegraphics[scale=0.6]{graphs/new/gabung-korelasi-p-rho.eps}
%\end{center}
...
%\begin{figure}[tb]
%\begin{center}
%\includegraphics[scale=0.6]{graphs/new/gabung-korelasi-p-rho-4region.eps}
%\end{center}
...
\begin{figure*}[!t]
\begin{center}
\subfloat[Scatter plot of $p$ value vs $\rho$ value. 
        Points in area 1 are the samples where a power-law with exponential cut-off is a more plausible model than a power-law.
        In area 3, a pure power-law may be more plausible than power-law with exponential cut-off, while in area 2 the results are ambiguous.]{
{\includegraphics[width=3.1in]{graphs/new/gabung-korelasi-p-rho.eps}}
{\label{fig:scatter-pvalue-vs-rho}}}
\subfloat[Scatter plot of $p$ value vs log-likelihood ratio (LR) for $\rho < 0.1$. In this figure we define area 1: LR=positive and $p$ value $<0.1$, area 2: LR=positive and $p$ value $>0.1$, area 3: LR=negative and $p$ value $<0.1$, area 4: LR=negative and $p$ value $>0.1$. There are no points in area 1 and area 2, meaning that the power-law model is not a better model for all data; instead $42\%$ of the points lie in area 3 and $58\%$ of the points lie in area 4.]{
{\includegraphics[width=3.1in]{graphs/new/gabung-korelasi-p-rho-4region.eps}}
{\label{fig:scatter-pvalue-vs-lr-for-rho-le-01}}}
\caption{Scatter plot $p$ values vs log-likelihood ratio (LR)}
\label{fig:scatter-vs-lr}
\end{center}
\end{figure*}
...
%\begin{figure}[tb]
%\begin{center}
%\includegraphics[scale=0.6]{graphs/non-nested/aggregate-p-cdf-non-nested-lognorm.eps}
%\end{center}
...
%\begin{figure}[tb]
%\begin{center}
%\includegraphics[scale=0.6]{graphs/non-nested/lr-p-lognorm.eps}
%\end{center}
...
\begin{figure*}[!t]
\begin{center}
\subfloat[CDF plot of $\rho$ value of log-likehood ratio test for power-law vs log-normal. Only $13\%$ $\rho$ values are less than $0.1$ therefore log-likelihood ratio sign is reliable only for $13\%$ of data points and for the rest, we can not take conclusive answer since log-likelihood ratio sign is not reliable indicator.]{
{\includegraphics[width=3.1in]{graphs/non-nested/aggregate-p-cdf-non-nested-lognorm.eps}}
{\label{fig:cdf-p-lognorm}}}
\subfloat[Scatter plot of log-likehood ratio for power-law vs log-normal for $\rho$ values less than $0.1$. All of log-likelihood ratio values sign are negative therefore log-normal is not a better fit.]{
{\includegraphics[width=3.1in]{graphs/non-nested/lr-p-lognorm.eps}}
{\label{fig:scatter-lognorm}}}
\caption{CDF plot of $\rho$ value  and Scatter plot $p$ values vs log-likelihood ratio (LR) for likelihood test for power-law vs log-normal.}
\label{fig:non-nested-lognorm}
\end{center}
\end{figure*}

Figure \ref{fig:cdf-p-lognorm} shows the CDF of the $\rho$ value for log-likehood ratio between power-law and log-normal.  
Less than $13\%$ of the $\rho$-values are less than $0.1$ therefore log-likelihood ratio sign is reliable only for $13\%$ data points.
Beyond of that, we can not take conclusive answer since log-likehood ratio sign is not reliable indicator. 
...
%\begin{figure}
%\begin{center}
%\includegraphics[scale=0.6]{graphs/non-nested/aggregate-p-cdf-non-nestedi-exp.eps}
%\end{center}
...
%\begin{figure}[tb]
%\begin{center}
%\includegraphics[scale=0.6]{graphs/non-nested/lr-pi-exp.eps}
%\end{center}
...
\begin{figure*}[!t]
\begin{center}
\subfloat[CDF plot of $\rho$ value of log-likehood ratio test for power-law vs exponential. Only $5.5\%$ $\rho$ values are less than $0.1$ therefore log-likelihood ratio sign is reliable only for $5.5\%$ of data points and for the rest, we can not take conclusive answer since log-likelihood ratio sign is not reliable indicator.]{
{\includegraphics[width=3.1in]{graphs/non-nested/aggregate-p-cdf-non-nestedi-exp.eps}}
{\label{fig:cdf-p-exp}}}
\subfloat[Scatter plot of log-likehood ratio for power-law vs exponential for $\rho$ values less than $0.1$. All of log-likelihood ratio values sign are negative therefore exponential is not a better fit.]{
{\includegraphics[width=3.1in]{graphs/non-nested/lr-pi-exp.eps}}
{\label{fig:scatter-exp}}}
\caption{CDF plot of $\rho$ value  and Scatter plot $p$ values vs log-likelihood ratio (LR) for likelihood test for power-law vs exponential.}
\label{fig:non-nested-exp}
\end{center}
\end{figure*}


Figure \ref{fig:cdf-p-exp} shows the CDF of the $\rho$ values for log-likehood ratio between power-law and exponential. 
Less than $5.5\%$ of the $\rho$ values are less than $0.1$ therefore log-likelihood ratio sign is reliable only for $5.5\%$ data points.
Beyond of that, we can not take conclusive answer since log-likehood ratio sign is not reliable indicator.
...
Networks show a tendency for link formation between neighboring vertices.
This tendency is called \textit{clustering}.
Clustering reflects the topology robustness. 
...
\begin{figure}[tb]
\begin{center}
\includegraphics[scale=0.6]{graphs/new/cdf-clustering.eps}
\end{center}
...
\begin{figure}[tb]
\begin{center}
\includegraphics[scale=0.6]{netscicom-graphs/simula.eps}
\end{center}
...
However, little work has been done on confirmingthat such controlled experiments correspond to the insitu system. %that topology in the real world. 
